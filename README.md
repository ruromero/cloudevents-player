...
2022-06-24 18:39:08,216 INFO  [io.quarkus] (main) cloudevents-player 1.2-SNAPSHOT on JVM (powered by Quarkus 2.16.1.Final) started in 0.879s. Listening on: http://0.0.0.0:8080
...
2022-06-24 18:51:43,513 INFO  [io.quarkus] (Quarkus Main Thread) cloudevents-player 1.2-SNAPSHOT on JVM (powered by Quarkus 2.16.1.Final) started in 2.543s. Listening on: http://localhost:8080
...
```shell script
$ ./target/cloudevents-player-1.2-SNAPSHOT-runner -Dplayer.mode=LOCAL
...
2022-06-24 18:48:11,565 INFO  [io.quarkus] (main) cloudevents-player 1.2-SNAPSHOT native (powered by Quarkus 2.16.1.Final) started in 0.022s. Listening on: http://0.0.0.0:8080
...
```{bash}
./target/cloudevents-player-1.2-SNAPSHOT-runner
```
...
# Knative Mode
./target/cloudevents-player-1.2-SNAPSHOT-runner -Dplayer.mode=KNATIVE
```
...
```bash
./target/cloudevents-player-1.2-SNAPSHOT-runner -Dbroker.uri=http://some-broker:1234
```
...
```bash
# The broker URL
./target/cloudevents-player-1.2-SNAPSHOT-runner -Dbroker.name=example -Dbroker.namespace=other
...
